...
import { Text, View, StyleSheet, SafeAreaView, StatusBar, TouchableOpacity, TextInput, Modal, Image, ScrollView, Keyboard, ActivityIndicator, Dimensions } from 'react-native'
...
export default class Add_advertisement extends Component {
    ...
    _getAdvertivenData = async () => {
        if (this.state.isConnected === true) {
            ...
            let url = config.baseURL + "advertisement_details.php?user_id_post=" + user_id_post + "&advertisement_id=" + this.state.advertisement_id + "&for_edit=forEdit" + "&country_code=965";
            console.log('url--', url);
            apifuntion.getApi(url).then((obj) => {
                ...
                if (obj.success == 'true') {
                    ...
                    let adver_arr = obj.adver_arr;
                    var ind = this.state.hoursData.findIndex(x => x.hours == parseInt(adver_arr.minimum_hours));
                    this.state.hoursData[ind].status = true;
                    var ind1 = this.state.hoursData1.findIndex(x => x.hours == parseInt(adver_arr.idle_time));
                    this.state.hoursData1[ind1].status = true;

                    this.setState({
                        trip_type_arr: obj.trip_type_arr,
                        boat_arr: obj.boat_arr,
                        img_arr: obj.adver_arr.img_arr,
                        city_arr: obj.city_arr,
                        city_arr1: obj.city_arr,
                        adver_arr: obj.adver_arr,
                        hoursData: [...this.state.hoursData],
                        hoursData1: [...this.state.hoursData1],
                        title_ar: adver_arr.adver_name[1],
                        title_eng: adver_arr.adver_name[0],
                        selected_city: adver_arr.city_name[config.language],
                        mobile: adver_arr.mobile.toString(),
                        trip_type_id: adver_arr.trip_type_id,
                        trip_type_id: adver_arr.trip_type_id,
                        trip_type_name: adver_arr.trip_type_name[config.language],
                        boat_id: adver_arr.boat_id,
                        boat_name: adver_arr.boat_name,
                        no_of_people: adver_arr.no_of_people.toString(),
                        location_address: adver_arr.location_address,
                        location_address: adver_arr.location_address,
                        discription_en: adver_arr.discription_arr[0],
                        discription_ar: adver_arr.discription_arr[1],
                        rental_price: adver_arr.rental_price,
                        extra_price: adver_arr.extra_price,
                        minimum_hours: adver_arr.minimum_hours,
                        idle_hours: adver_arr.idle_time,
                        discount: adver_arr.discount,
                        selected_city_id: adver_arr.city_get,
                    });

                    config.mapaddress = adver_arr.location_address;
                    config.maplat = adver_arr.location_lat;
                    config.maplong = adver_arr.location_lng;

                    if (obj.adver_arr.img_arr != 'NA') {
                        let len = obj.adver_arr.img_arr.length;
                        let rem_len = 8 - len;

                        for (let i = 0; i < len; i++) {
                            let a = config.img_url1 + obj.adver_arr.img_arr[i].image;
                            let id = obj.adver_arr.img_arr[i].adv_img_id;
                            let json = {
                                image: a,
                                id: id,
                            }
                    ...
                    // alert(JSON.stringify(this.state.show_img))
                } else {
    ...
    _delete_image = async (id, index) => {
        if (this.state.isConnected === true) {
            ...
            let url = config.baseURL + "image_delete.php?user_id_post=" + user_id_post + "&image_id=" + id;
            console.log('url--', url);
            apifuntion.getApi(url).then((obj) => {
                this.setState({ loading: false });
                return obj.json();
    ...
    _submitAdvertisement = async () => {
        this.scrollViewRef.scrollResponderScrollTo({ y: 50 });
        console.log('this.scrollViewRef',this.scrollViewRef);
        let { title_ar, title_eng, mobile, trip_type_id, trip_type_name, boat_id, boat_name, no_of_people, location_address, location_lat, location_lng, discription_ar, discription_en, rental_price, extra_price, minimum_hours, idle_hours, discount, advertisement_id, selected_city_id } = this.state;
        ...
        if (this.state.isConnected === true) {
            ...
            let url = config.baseURL + "advertisement_edit.php";
            var data = new FormData();
            data.append('user_id_post', user_id_post)
            data.append('advertisement_id_post', advertisement_id)
        ...
        return (
            <KeyboardAwareScrollView style={{ flex: 1, height: '100%', backgroundColor: '#ffffff', paddingLeft: 20, paddingRight: 20 }}>
                ...
                <View style={styles.header_earnig}>
                    ...
                    <Text style={styles.earnig_title}>{Lang_chg.edit_adver_txt[config.language]}</Text>
                    <Text></Text>
                </View>
                <ScrollView  ref={(ref) => { this.scrollViewRef = ref; }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps='handled'>
                    ...
                    {/* {
                            this.state.show_img.map((item, index) =>   height: 100,
        width: '30%',( */}
                    <FlatList
                            ...
                            return (
                                <TouchableOpacity style={{ height: 100, width: 100, margin: 10, borderRadius: 15, borderColor: '#d15400', borderWidth: 1 }} onPress={() => { (item.id == 0) ? this.setState({ img_type: index, editModalvisibal: true, }) : null }} activeOpacity={0.9}>
                                    ...
                                    }
                                    {
                                        ...
                                        <TouchableOpacity activeOpacity={0.9} style={{ position: 'absolute', right: 0, right: -3, top: -3, }} onPress={() => { this._removeImg(index, item.id) }}>
                    ...
                    {/* {
                            this.state.no_show_img.map((item1, index) => (
                                <View style={[styles.box_slide]}>
                                    <TouchableOpacity onPress={() => { this.setState({ img_type: 1, editModalvisibal: true, }) }}  >
                                        <Image resizeMode="contain" style={styles.slide_plus} source={require('./icons/plus_new.png')} />
                                    </TouchableOpacity>
                                </View>
                            ))
                        } */}
